...
namespace collisions {
	...
	: isCircle_(false) 
	{	
	    vertx_ = (float*)calloc(4, sizeof(float));
		verty_ = (float*)calloc(4, sizeof(float));
	    vertx_[0] = x - w/2.f;
	    verty_[0] = y - h/2.f;
	    vertx_[1] = x + w/2.f;
	    verty_[1] = y - h/2.f;
	...
	void Collidable::updateNormals() {
		normx_ = (float*)calloc(nvert_, sizeof(float));
		normy_ = (float*)calloc(nvert_, sizeof(float));
		for (int i = 0; i < nvert_-1; ++i) {
	        normx_[i] = verty_[i+1] - verty_[i];
	        normy_[i] = -vertx_[i+1] + vertx_[i];
		}
	    normx_[nvert_-1] = verty_[0] - verty_[nvert_-1];
	    normy_[nvert_-1] = -vertx_[0] + vertx_[nvert_-1];
	}
	...
	void Collidable::rotateVertices(glm::vec2 position, float angle)
	{
	    ...
	    for (int i = 0; i < nvert_; ++i){
	        ...
	        vertx_[i] = position.x + (s.x * cosr - s.y * sinr);
	        verty_[i] = position.y + (s.x * sinr + s.y * cosr);
	    }
		updateNormals();
	}
	...
	float dist_poly_circ(int nvert, float* vertx, float* verty, float circx, float circy);
	float dist_poly_circ(int nvert, float* vertx, float* verty, float circx, float circy, glm::vec2* closestPnt, uint* faceIndex);
	float dist_circ_circ(float ax, float ay, float bx, float by);
	...
	float dist_poly_circ(uint nvert, float* vertx, float* verty, float circx, float circy)
	{
	...
	float dist_poly_circ(uint nvert, float* vertx, float* verty, float circx, float circy, glm::vec2* closest_point, uint* face_index)
	{
	    float distance = FLT_MAX;
	    float temp;
	    for (int i = 0; i < nvert - 1; ++i){
